...
class MDStreamRenderer:
    ...
    def _add_snap(self, lang, code):
        ...
        console.print(
            Text(f"   snap {cid}  ", justify="right", style="#e6db74 on #272822"),
            Text(f"", justify="right", style="#272822"),
            sep="",
        )
    ...
    def _update(self, edl: str = ""):
        ...
        try:
            # 使用自定义主题的 Markdown
            self.md = Markdown(
                self.content, code_theme="monokai", inline_code_lexer="text"
            )
        ...
        if cmd[:6] == "change":
            ...
            for sid in range(len(self.history["snap"])):
                ...
                if cmd == "" or snap["lang"] == cmd:
                    print(
                        f"│   $S{sid:<3} [{snap['lang']:10}]: {self.short(snap['code'])!r}"
                    )
            print("╰─────────────")
            return "done"
        else:
